...
|---|--------|-------|----------------|-------------------|
| 1 | País | `pais` | `id_pais` (Long) | — |
| 2 | Familia de producto | `producto_familia` | `id_producto_familia` (Long) | — |
| 3 | Tienda | `tienda` | `id_tienda` (Long) | — |
| 4 | Ciudad | `ciudad` | `id_ciudad` (Long) | País |
| 5 | Departamento de producto | `producto_departamento` | `id_producto_departamento` (Long) | Familia de producto |
| 6 | Vendedor | `vendedor` | `id_vendedor` (Long) | Tienda (obligatoria) |
| 7 | Cliente | `cliente` | `id_cliente` (Long) | Ciudad |
| 8 | Categoría de producto | `producto_categoria` | `id_producto_categoria` (Long) | Departamento de producto |
| 9 | Producto | `producto` | `id_producto` (Long) | Categoría de producto |
| 10 | Venta | `venta` | `id_venta` (Long) | Tienda, Cliente, Vendedor (obligatorias) |
| 11 | Detalle de venta | `venta_detalle` | `id_venta_detalle` (Long) | Venta, Producto |

> Todas las PKs son `Long` con generación por secuencia (`@GeneratedValue(strategy = SEQUENCE)`). Cada tabla tiene su propia secuencia PostgreSQL.
...
### 5.2 Generación de ids (secuencias PostgreSQL)

Todas las PKs usan `Long` con `@GeneratedValue(strategy = SEQUENCE)`, una secuencia PostgreSQL por tabla:
...
- Las secuencias se crean directamente en PostgreSQL (ya existen en la base de datos).
- JPA se encarga de asignar el id automáticamente al hacer `save()`.
...
### 5.10 Restaurar base de datos desde backup

El esquema completo de la base de datos (con datos) se encuentra en `backup_nexa_store.sql`. Para restaurarlo:

```bash
psql -U postgres -d nexa_store_db -f backup_nexa_store.sql
```

Si la base de datos no existe, crearla primero:

```bash
createdb -U postgres nexa_store_db
psql -U postgres -d nexa_store_db -f backup_nexa_store.sql
```
